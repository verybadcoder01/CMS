...
* /api/users/groups - список всех зарегестрированных групп. Первое, что видит пользователь, заходящий на страницу. Возвращает json, содержащий список номеров [3](#group-json) \
    ...
    - 500 
    - валидный ответ (200)
* /api/admins/home - домашняя страница админов (доступна только после логина). Возвращает время до истечения сессии, json [7](#time-json) \
    Возможные коды:
...
### Time json
Содержит 1 ключ "untilExpires" - время в секундах. \
Пример: ```{"untilExpires": 54}```
